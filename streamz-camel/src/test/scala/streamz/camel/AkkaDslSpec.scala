...
class AkkaDslSpec extends TestKit(ActorSystem("test")) with WordSpecLike with Matchers with BeforeAndAfterAll {
  ...
  "A receiver" must {
    "create a stream" in {
      ...
      Source.fromGraph(receiveBody[Int]("seda:q1")).take(3).runWith(Sink.seq[Int]).await should be(Seq(1, 2, 3))
    }
    "complete with an error if type conversion fails" in {
      producerTemplate.sendBody("seda:q2", "a")
      intercept[TypeConversionException](Source.fromGraph(receiveBody[Int]("seda:q2")).runWith(Sink.ignore).await)
    }
  }

  "A sender" must {
    "send to an endpoint and continue with the sent message" in {
      Source.fromGraph(receiveBody[Int]("seda:q3")).send("seda:q4").take(3).runWith(Sink.seq[Int]).onComplete {
